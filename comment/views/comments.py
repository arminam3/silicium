...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        article = get_object_or_404(Article, id=self.comment.content_object.id)
        ...
        if self.comment.parent:
            ...
            if parent_email in [user_email, author_email]:
                parent_email = False

        if author_email:
            email = EmailMessage(
                "دیدگاه جدید",
                f"دبدگاه جدیدی برای مقاله {article}  ارسال شد "
                f"\n برای مشاهده صفحه کلیک کنید : "
                f"{current_site}{reverse('blog:article_detail', args=[article.slug,])}",
                # 'arminam3214@gmail.com',
                to=[author_email]
            )
            email.send()
        if user_email:
            email = EmailMessage(
                "ارسال دیدگاه شما",
                f"دیدگاه شما با موفقیت دریافت شد . در صورت نیاز به شما پاسخ می دهیم",
                # 'arminam3214@gmail.com',
                to=[user_email]
            )
            email.send()
        if parent_email:
            email = EmailMessage(
                "پاسخ برا یدیدکاه شما",
                f"یک پاسخ برا یدیدگاه شما در مقاله {article} ثبت شد"
                f"\n برای مشاهده صفحه کلیک کنید : "
                f"{current_site}{reverse('blog:article_detail', args=[article.slug, ])}",
                # 'arminam3214@gmail.com',
                to=[parent_email]
